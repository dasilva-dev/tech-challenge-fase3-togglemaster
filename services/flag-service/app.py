...
def ensure_schema():
    """ Garante que o schema do flag-service exista mesmo em volumes antigos """
    conn = None
    cur = None
    try:
        conn = pool.getconn()
        ...
        cur.execute("DROP TRIGGER IF EXISTS set_timestamp ON flags")
        cur.execute(
            """
        ...
        conn.commit()
        log.info("Schema do flag-service garantido com sucesso.")
    except Exception as e:
        if conn:
            conn.rollback()
        log.critical(f"Falha ao garantir schema do flag-service: {e}")
        sys.exit(1)
    finally:
        if cur:
            cur.close()
