...
def ensure_schema():
    """ Garante que o schema do targeting-service exista mesmo em volumes antigos """
    conn = None
    cur = None
    try:
        conn = pool.getconn()
        ...
        cur.execute("DROP TRIGGER IF EXISTS set_timestamp ON targeting_rules")
        cur.execute(
            """
        ...
        conn.commit()
        log.info("Schema do targeting-service garantido com sucesso.")
    except Exception as e:
        if conn:
            conn.rollback()
        log.critical(f"Falha ao garantir schema do targeting-service: {e}")
        sys.exit(1)
    finally:
        if cur:
            cur.close()
